...
const myFavSong :{ song :string , singer :string } = { song : '서른즈음에' , singer : '김광석'};
...
function 리터럴가위바위보( 손 :'가위'|'바위'|'보') : ('가위'|'바위'|'보')[]{
  let 리턴값:('가위'|'바위'|'보')[] = [];
  리턴값.push(손);
  return 리턴값;
}
...
*/
type 숙제3함수타입 = (a:string,func1:CutZeroType,func2:RemoveDashType) => number | string;
...
class Person {
  name : string;
  constructor(a :string){   // 복제되는게 항상 object타입이므로 return 타입을 지정할 필요없음
    this.name  = a;
  }

  펄슨함수(a:string):string{ // prototype 함수도 타입지정해서 쓰셈
    return '안녕' + a;
  }
}
...
function 함수6(...a:number[]){         // rest parameter도 type지정 해야함
...
function 함수11(a: string | undefined){
  if( a && typeof a === 'string'){        // undefined면 if문 실행안됨. string이면 if문 실행됨.
...
function 함수14() :never{
  ...
  while(true){      // while(true)처럼 계속 돌거나
...
class User5{
  ...
  public 함수(){ // 이렇게 클래스 안에 있는 메서드에도 public 붙이기 가능
    alert("배찬바보");
  }
}
...
function 제네릭숙제1<제네릭타입 extends string | string[]>( 문자 : 제네릭타입):void{
...
let 사먹은음식2:[string,number, ...boolean[]] = ['동서녹차', 4000, true, false, true, true, false, true];
